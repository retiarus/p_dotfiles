...
httpc() {
  http \
    --cert=/home/peregrinus/.certs/client-wr-mqtt.crt \
    --cert-key=/home/peregrinus/.certs/client-wr-mqtt.key \
    POST https://cubietruck.falcolabs.net:8001/api/publish \
    $*
}
...
if [ $topic = "work_room/switch_sound_all" ]; then
  httpc topic=work_room/switch_sound_0 message=$message --print=HBhb
  httpc topic=work_room/switch_sound_1 message=$message --print=HBhb
  httpc topic=work_room/rele_board_0 message="3:$message" --print=HBhb
  httpc topic=work_room/rele_board_1 message="0:$message" --print=HBhb
